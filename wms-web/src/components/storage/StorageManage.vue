...
export default defineComponent({
  ...
  methods: {
    ...
    del(id){
      this.$axios.get('http://localhost:8090/storage/delete?id=' + id).then(res=>res.data).then(res => {
        console.log(res)
        if (res.code === 200) {
          this.$message({
            message: '删除成功',
    ...
    save(){
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.$axios.post('http://localhost:8090/storage/saveOrUpdate', this.form).then(res=>res.data).then(res => {
            console.log(res)
            if (res.code === 200) {
              this.$message({
                message: '操作成功',
...
<template>
  <div>
    <div style="margin-bottom: 20px;">
      <el-input
          v-model="name" placeholder="请输入仓库名" suffix-icon="el-icon-search" style="width: 200px"
